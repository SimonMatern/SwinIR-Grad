...
class PixelShuffle3d(nn.Module):
    ...
    def forward(self, input):
        ...
        D, P1, P2 = self.scale
        nOut = channels // (D* P1 * P2)

        out_depth = in_depth * D
        out_height = in_height * P1
        out_width = in_width * P2

        input_view = input.contiguous().view(batch_size, nOut, D, P1, P2, in_depth, in_height, in_width)
...
class SwinBlock(nn.Module):
    ...
    def __init__(self, in_dim = 64, depth=4, heads=8, patch_size=(2,4,4), window_size = (1,7,7)):
        ...
        self.tokenize = PatchEmbed3D(patch_size=patch_size, in_chans=in_dim, embed_dim=D*P*P*in_dim)
        #self.shuffle = nn.PixelShuffle(P)
        self.shuffle = PixelShuffle3d(patch_size)
        self.transformer_encoder = BasicLayer(dim=D*P*P*in_dim, depth=depth, num_heads=heads, window_size = window_size)
    ...
    def forward(self, xs):
        ...
        y = y*xs                                #
        #y = rearrange(y, 'b c s h w -> b s c h w')
        y = self.shuffle(y)                     # (B, C*D*P*P,S/D H/P, W/P) -> (B, C, S, H, W)
        y = rearrange(y, 'b c s h w -> b s c h w')
        #y = rearrange(y, ' b (c P1 P2 D) s h w -> b (s D) c (P1 h) (P2 w)',P1=self.patch_size[1], P2=self.patch_size[1],D=self.patch_size[0])
        #y = rearrange(y, '(b s) c h w -> b s c h w', s=S)
        return y